...
class Master_Franchise_pod extends CI_Controller 
{
	...
	public function index()
	{
        ...
        $this->load->view('masterfranchise/pod/view_pod',$data);
	...
	public function addpod()
	{
		...
		$data['message']="";
	    $this->load->view('masterfranchise/pod/addpod',$data);
	}
	public function insertpod()
	{
		...
		if (!empty($all_data)) 
		{
			...
			// echo '<pre>';print_r($_SESSION['customer_name']);die;
			...
			$curret >= date('Y-m-d', strtotime($this->input->post('booking_date'))))
		   {
			...
			$date=date('y-m-d');
			    $r= array('id'=>'',
					  'deliveryboy_id'=>$_SESSION['customer_name'],
				...
				if ($this->db->affected_rows()>0) {
					$msg = 'Pod Uploaded Successfully';
						$class = 'alert alert-succcess alert-dismissible';
				}else{
			...
			else
			{
			    ...
			    $this->session->set_flashdata('notify', $msg);
				$this->session->set_flashdata('class', $class);
                redirect('master-franchise/add-pod');
		}
	}
	...
	public function uploloadPOD($data,$file){
		...
		curl_setopt_array($curl, array(
		  ...
		  CURLOPT_POSTFIELDS => array('pod_no' => $data['pod_no'],'image'=> new CURLFILE($file['tmp_name']),'user_id' => '1'),
	...
	{
		$data['message']="";
	    $this->load->view('masterfranchise/pod/uploadbulkpod',$data);
	}
	...
	function pending_pod()
	{
		...
		$this->load->view('masterfranchise/pod/view_pendingpod', $data);
	}
