...
class Franchise_deliverysheet extends CI_Controller
{
...
GROUP BY deliverysheet_id order by tbl_domestic_deliverysheet.id desc");
		...
		$this->load->view('franchise/deliverysheet/view_deliverysheet', $data);
	}
	public function adddelivery()
	{
		...
		$data['did'] = $id;
		$this->load->view('franchise/deliverysheet/addeliverysheet', $data);
	}
	...
	public function awbnodata()
	{
		...
		if ($empty['STATUS'] == 'Out For Delivery') {
			...
			WHERE tbl_domestic_booking.pod_no='$pod_no' and tbl_domestic_booking.is_delhivery_complete = '0' AND tbl_domestic_menifiest.destination_franchise = '$franchise_id' AND tbl_domestic_bag.bag_recived = '1' and tbl_domestic_menifiest.reciving_status = '1' ORDER BY tbl_domestic_bag.id DESC LIMIT 1");
			$data = "";
				...
				if (empty($undeliverd)) {
					...
					if (
						...
						$data = array(
							...
							'pod_no' => $pod_no,
							'status' => 'recieved',
							'delivery_date' => $this->input->post('datetime'),
						);
	...
	public function deliverysheet_detail($id)
	{
		...
		$this->load->view('franchise/deliverysheet/view_deliverysheet_detail', $data);
	}
	...
	public function deliverysheet($deliverysheet_id = '')
	{
		...
		$this->load->view('franchise/deliverysheet/printdelivery', $data);
	}
	...
	public function print_deliverysheet($deliverysheet_id = '')
	{
		...
		$this->load->view('franchise/deliverysheet/printprintdelivery', $data);
	}
	...
	public function update_drs()
	{
		...
		if ($this->input->post('pod_no')) {
			...
			redirect('franchise/update-drs');
		}

		$this->load->view('franchise/deliverysheet/update_drs', $data);
	}
